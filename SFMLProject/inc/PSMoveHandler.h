...
public:
	...
	void identify(int globalId, bool on) override
	{
		int controllerId = 0;
	...
	PSMQuatf ExtractPSMoveYawQuaternion(const PSMQuatf& q)
	{
		...
		// Convert this yaw rotation back into a quaternion
		PSMVector3f eulerPitch = {static_cast<float>(1.57079632679489661923), 0.f, 0.f}; // pitch 90 up first
	...
	void alignPSMoveAndHMDTrackingSpace(PSMController* controller)
	{
		...
		if (m_PSMControllerType == PSMController_Move)
		{
			...
			PSMVector3f eulerPitch = {static_cast<float>(M_PI_2), 0.0f, 0.0f};
		...
		else if (m_PSMControllerType == PSMController_DualShock4 ||
			m_PSMControllerType == PSMController_Virtual)
		{
			// Translation) The controller's position is a few inches ahead of the HMD's on the HMD's local -Z axis. 
			controllerLocalOffsetFromHmdPosition = {0.0f, 0.0f, -1.0f * m_fControllerMetersInFrontOfHmdAtCalibration};
		}
	...
	*/
		...
		controller_pose_meters.Position =
			PSM_Vector3fScale(&controller_pose_meters.Position, k_fScalePSMoveAPIToMeters);
		...
		if (m_PSMControllerType == PSMController_Move)
		{
			...
			{
				const PSMVector3f eulerPitch = {static_cast<float>(M_PI_2), 0.0f, 0.0f};
		...
		PSMPosef test_composed_controller_world_space = PSM_PosefConcat(
			&controller_pose_meters, &driver_pose_to_world_pose);
	...
	vr::DriverPose_t getDriverPose(int controllerId)
	{
		...
		case PSMController_Virtual:
			{
				const PSMVirtualController& view = v_controllers[controllerId].controller->ControllerState.
				                                                               VirtualController;
		...
		default:
			LOG(FATAL) << "!!! getDriverPose called on a PSMoveHandler device with no known type !!!";
			// Not supposed to happen, and may be impossible
			break;
		}
	}
		...
		const bool& poseIsValid)
	{
		...
		if (m_fVirtuallExtendControllersYMeters != 0.0f || m_fVirtuallExtendControllersZMeters != 0.0f)
		{
			...
			PSMVector3f shift = {
				static_cast<float>(m_Pose.vecPosition[0]), static_cast<float>(m_Pose.vecPosition[1]),
				static_cast<float>(m_Pose.vecPosition[2])
			};
...
private:
	...
	// HMD *may* influence the magnetic field and disrupt readings
