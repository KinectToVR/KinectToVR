...
public:
	...
	virtual void identify(int controllerId, bool on)
	{
	} // Shows the user which device is being referred to, i.e. flashing light, rumble for a short while

	bool active = false;
	// By default, device trackers should be off, and spawned into the vector, but disabled - initialising them flips the bool, and allows them to be updated

	std::string identifyDeviceString = "Identify Me!";
	// String used for the button to show for the identify function in the GUI - can be overidden by a device handler to fit the method used
